...
module.exports = (instance, options, next) => {
    ...
    */
    ...
    try {
      ...
      for (let i = 0; i < goods.length; i++) {
        ...
        goods[i].in_stock = 0;
        let item_reminder = 0;
        const SERVICES = request.body && request.body.services
      ...
      instance.oauth_admin(request, reply, async (admin) => {
        ...
        if (request.body.service) {
          try {
            ...
            const services = await instance.services
              .find({ organization: admin.organization }, { _id: 1, name: 1 })
              .lean()
  ...
  instance.get('/goods/sales/get_deleted/:time', { version: '1.1.0' }, (request, reply) => {
    instance.authorization(request, reply, async (user) => {
      ...
      let deleted_items = []
      try {
        deleted_items = await instance.deletedGoodsSales
          .find({ organization: user.organization, date: { $gte: time } })
          .lean()
      } catch (error) { }
      const resp = []
  ...
  const get_file = async (request, reply, admin) => {
    if (admin.ui_language && admin.w.value != undefined) {
    ...
    const titles = [
      '_id', 'category_id', 'sku', 'name',
      'parent_category', 'category', 'sold_by',
