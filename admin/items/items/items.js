...
module.exports = (instance, options, next) => {
  ...
  const itemSchema = {
    body: {
      type: 'object',
      required: ['has_variants', 'variant_options', 'variant_items', 'services'],
      properties: {
        price: { type: 'number' },
        price_currency: {
          type: 'string',
  ...
  instance.post('/items/create/with-variant', {
    ...version,
    schema: itemSchema
  }, (request, reply) => {
    ...
    instance.oauth_admin(request, reply, (admin) => {
      if (admin) {
        instance.check_sku_and_category(request, admin.organization, (result) => {
          if (result.success) {
  ...
  instance.post('/items/update/with-variant/:id', {
    ...version,
    schema: itemSchema
  }, (request, reply) => {
  ...
  const getItemVariants = async (request, reply, admin) => {
    try {
      ...
      const variants = await instance.goodsSales.find({ _id: { $in: item.variant_items } })
      for (let i = 0; i < variants.length; i++) {
        ...
        if (services && services.length > 0 && typeof variants[i].services == typeof []) {
  ...
  const get_list_of_items = async (request, reply, admin) => {
    ...
    try {
      if (parseInt(request.params.limit)) {
        limit = parseInt(request.params.limit) != 0 ? parseInt(request.params.limit) : 1
      }
      else {
        limit = 200
      }
    ...
    */
    ...
    const org_services = await instance.services.find({ organization: admin.organization })
    const servicesObj = {}
    ...
    try {
      const goods = await instance.goodsSales.aggregate(pipeline).allowDiskUse(true);
      ...
      for (let i = 0; i < goods.length; i++) {
        ...
        goods[i].in_stock = 0;
        let item_reminder = 0;
        const SERVICES = request.body && request.body.services ? request.body.services : []
        ...
        for (const service of goods[i].services) {
          ...
          if (servicesObj[service.service + '']) {
            if (SERVICES && SERVICES.length > 0 || SERVICE != '') {
              if (SERVICES.includes(service.service + '') || SERVICE == service.service + '') {
        ...
        if (request.body) {
          if (request.body.services != '' || request.body.service != '') {
            ...
            for (var s of goods[i].services) {
              goods[i].stock_status.low_stock += (s.low_stock > s.in_stock && s.in_stock > 0) ? 1 : 0
        ...
        if (
          request.body.stock == 'low' && goods[i].stock_status.low_stock > 0
          || request.body.stock == 'out' && goods[i].stock_status.out_of_stock > 0
          || request.body.stock == 'positive' && goods[i].stock_status.position > 0
      ...
      for (let i = 0; i < goods.length; i++) {
        if (goods[i].has_variants) {
          goods[i].in_stock = await instance.calculateInStockItemVariants(goods[i].variant_items, request.body && request.body.service ? request.body.service : '')
          if (goods[i].sold_by == 'box' || goods[i].sold_by == 'pcs') {
            goods[i].reminder = await instance.calculateReminderItemVariants(goods[i].variant_items, request.body && request.body.service ? request.body.service : '')
          }
        }
      }
  ...
  instance.post('/items/list_of_items/:limit/:page', version, (request, reply) => {
    instance.oauth_admin(request, reply, (admin) => {
      if (admin) { get_list_of_items(request, reply, admin) }
    })
  })
  ...
  const list_of_items = (request, reply, admin) => {
    ...
    if (request.body) {
      ...
      if (request.body.search != undefined) {
        find['$or'] = [
          ...
          {
            name: { $regex: (instance.converter(request.body.search) != "" ? instance.converter(request.body.search) : "salom_dunyo_ishla_qale"), $options: 'i' }
          }
        ]
      }
    }
    instance.goodsSales.find(find, query, async (err, goods) => {
      ...
      if (request.headers['accept-user'] == 'boss' || request.headers['accept-user'] == 'admin') {
        for (let i = 0; i < goods.length; i++) {
          try {
        ...
        if (err) {
          ...
          for (var g of goods) {
            ...
            for (var s of g.services) {
              if (s.in_stock && s.service + '' == request.params.service + '' || request.params.service == '') {
                in_stock += s.in_stock
              }
            }
            g.in_stock = in_stock
  ...
  instance.get('/items/list_of_items/:service', {
    ...version,
    schema: {
      params: {
        type: 'object',
  ...
  instance.get('/items/get_available_sku', version, (request, reply) => {
    instance.authorization(request, reply, (admin) => {
      if (admin) { get_sku_new(request, reply, admin) }
  ...
  const create_item = (data, admin, reply) => {
    ...
    var $model = new instance.goodsSales(Object.assign({ organization: admin.organization }, data))
    instance.services.find({
      ...
      if ($model.services) {
        if ($model.services.length > 0) {
          for (let i = 0; i < $model.services.length; i++) {
            if ($model.services[i].service != '' || $model.services[i].service != null) {
              serviceObj[$model.services[i].service + ''] = $model.services[i]
        ...
        if ($model.composite_items) {
          if ($model.composite_items.length > 0) {
            ...
            for (let i = 0; i < $model.composite_items.length; i++) {
              if ($model.composite_items[i].product_id != '' || $model.composite_items[i].product_id != null) {
                var quality = 0
                if (parseFloat($model.composite_items[i].quality)) {
                  quality = parseFloat($model.composite_items[i].quality)
                }
                valid_items.push({
                  product_id: instance.ObjectId($model.composite_items[i].product_id),
                  product_name: $model.composite_items[i].product_name ? $model.composite_items[i].product_name : "",
                  quality: quality
                })
              }
            }
      ...
      if (!($model.is_track_stock || ($model.is_composite_item && $model.use_production))) {
        for (let i = 0; i < $model.services.length; i++) {
      ...
      $model.save((err, item) => {
        ...
        if (err) {
          ...
          reply.ok(item)
          if (item.is_track_stock || (item.is_composite_item && item.use_production)) {
            if (item.services.length > 0) {
              var time = new Date().getTime()
              for (var s of item.services) {
                if (s.in_stock != 0 && s.in_stock != null) {
                  item.in_stock = s.in_stock
                  instance.create_inventory_history(admin, "item edit", "", s.service, item._id, item.cost, item.in_stock, item.in_stock, time)
                }
                if (typeof s.reminder == typeof 5 && s.reminder > 0) {
                  instance.create_reminder_history(admin, "item edit", "", s.service, item._id, item.cost, 0, s.reminder, time)
                }
              }
            }
          }
      ...
      if (item1) {
        ...
        if (data.is_composite_item && data.composite_items) {
          ...
          for (let i = 0; i < data.composite_items.length; i++) {
            if (data.composite_items[i].product_id != "" && data.composite_items[i].product_id != null) {
              var quality = 0
              ...
              data.composite_items[i].product_id = instance.ObjectId(data.composite_items[i].product_id)
          ...
          if (result.ok) {
            ...
            if (data.services) {
              for (var s of data.services) {
                ...
                if (!s.available) {
                  instance.push_deleted_items({ headers: {} }, [id], admin.organization, s.service)
                }
              }
            }
            // create inv history
            ...
            instance.goodsSales.findOne({
              ...
              if (item2) {
                ...
                for (let i = 0; i < item2.services.length; i++) {
                  ...
                  if (item2.services[i].in_stock != item1.services[i].in_stock) {
                    instance.create_inventory_history(admin, "item edit", "", item2.services[i].service, item2._id, item2.cost, item2.services[i].in_stock - item1.services[i].in_stock, item2.services[i].in_stock, time)
                  }
                  if (item2.services[i].reminder != item1.services[i].reminder && (typeof item2.services[i].reminder == typeof 5 || typeof item1.services[i].reminder == typeof 5)) {
                    instance.create_reminder_history(admin, "item edit", "", item2.services[i].service, item2._id, item2.cost, item1.services[i].reminder, item2.services[i].reminder, time)
                  }
                  // price
                  ...
                  if (item1.services[i].price != item2.services[i].price) {
                  ...
                  if (item1.services[i].prices != item2.services[i].prices) {
      ...
      instance.oauth_admin(request, reply, async (admin) => {
        ...
        if (request.body.service) {
          try {
            ...
            const item = await instance.goodsSales.findOne({ _id: request.params.id })
            const serviceMap = {}
            ...
            if (request.body.prices instanceof Array) {
              ...
              if (request.body.prices.length > 1) {
                if (request.body.prices[0].from == 0 && request.body.prices[1].from != 0) {
              ...
              if (request.body.prices.length > 2 && prices.length > 1) {
                if (request.body.prices[1].from == 0 && request.body.prices[2].from != 0) {
            ...
            for (const ser of services) {
              if (serviceMap[ser._id + '']) {
                ...
                if (service_id + '' == ser._id + '') {
                  if (typeof request.body.price == typeof 5 && request.body.price != serviceMap[ser._id + ''].price) {
                    updateItem.last_price_change = new Date().getTime()
                  }
                  if (prices instanceof Array && serviceMap[ser._id + ''].prices instanceof Array && !instance.equalPrices(prices, serviceMap[ser._id + ''].prices)) {
                    updateItem.last_price_change = new Date().getTime()
                  }
                  updateItem.services.push({
                    ...serviceMap[ser._id + ''],
                    price: request.body.price != undefined ? request.body.price : serviceMap[ser._id + ''].price,
                    prices: request.body.prices != undefined ? prices : serviceMap[ser._id + ''].prices,
                    in_stock: request.body && request.body.in_stock != undefined ? request.body.in_stock : serviceMap[ser._id + ''].in_stock,
                    reminder: request.body && request.body.reminder != undefined ? request.body.reminder : serviceMap[ser._id + ''].reminder
                  })
                }
                else {
                ...
                if (service_id + '' == ser._id + '') {
                  updateItem.services.push({
                    ...
                    price: request.body.price != undefined ? request.body.price : item.price,
                    prices: request.body.prices != undefined ? prices : [],
                    in_stock: request.body && request.body.in_stock != undefined ? request.body.in_stock : 0,
                    reminder: request.body && request.body.reminder != undefined ? request.body.reminder : 0,
                  })
                }
                else {
                  updateItem.services.push({
  ...
  const get_items_by_category_ids = (request, reply, user) => {
    ...
    }, { price: 1, sku: 1, barcode: 1, name: 1, services: 1, mxik: 1 }, (err, goods) => {
      if (err || goods == null) {
        goods = []
      }
      for (let i = 0; i < goods.length; i++) {
  ...
  instance.post('/get_items_by_category_ids', { version: '1.0.0' }, (request, reply) => {
    ...
    instance.authorization(request, reply, async (user) => {
      ...
      try {
        ...
        */
        let goods = [];
        // if (time) {
        goods = await instance.goodsSales.find(query).sort({ last_updated: -1 }).limit(100)
        // }
        // else {
  ...
  instance.get('/goods/sales/get_updated/:time',
    {
      version: '1.0.0'
    },
    getUpdatedHandler
  ...
  instance.get('/goods/sales/get_deleted/:time', { version: '1.1.0' }, (request, reply) => {
    instance.authorization(request, reply, async (user) => {
      let time = parseInt(request.params.time)
      let deleted_items = []
      try {
        deleted_items = await instance.deletedGoodsSales.find({ organization: user.organization, date: { $gte: time } })
      } catch (error) { }
      const resp = []
  ...
  const get_file = async (request, reply, admin) => {
    ...
    const titles = [
      '_id', 'category_id', 'sku', 'name', 'category', 'sold_by',
      ...
      instance.services.find({
        ...
        instance.goodsSales.aggregate([
          ...
          for (const g of goods) {
            ...
            for (let i = 0; i < 3; i++) {
              if (g.has_variants && g.variant_options && g.variant_options[i]) {
                if (typeof g.variant_options[i].option_name == typeof 'invan') {
            ...
            else if (typeof g.barcode == typeof 'invan' && g.barcode.length > 0) {
              good.push(g.barcode)
            }
            else {
              good.push('')
            ...
            if (g.primary_supplier_id && (g.primary_supplier_id + '').length > 0) {
              try {
                const supplier = await instance.adjustmentSupplier.findOne({ _id: g.primary_supplier_id + '' })
  ...
  const get_file_new = async (request, reply, admin) => {
    ...
    const titles = [
      '_id', 'category_id', 'category', 'sku', 'name', 'sold_by',
      'prices', 'price', 'cost', 'barcode', 'representation',
      // 'option_name1', 'option_value1',
      // 'option_name2', 'option_value2',
  ...
  const get_file_mxik = async (request, reply, admin) => {
    ...
    const titles = [
      '_id', 'category_id', 'category', 'sku', 'name', 'sold_by',
      'prices', 'price', 'cost', 'barcode', 'representation', "mxik",
      "nds_value"
    ]
    instance.organizations.findOne({ _id: organizationId }, (err, org) => {
      instance.services.findOne(
        ...
        async (_, service) => {
          ...
          const settingFeatures = await instance.settingFeatures.findOne({ organization: organizationId });
          // to_time = new Date().getTime()
          // from_time = to_time - 24 * 60 * 60 * 1000
          instance.goodsSales.aggregate([
            {
            ...
            for (const g of goods) {
              ...
              good.push(g.sold_by)
              let prices = ''
              const goodService = g.services.find(elem => elem.service == serviceId)
              const servicePrice = goodService.price ? goodService.price : g.price;
              ...
              if (settingFeatures.prices) {
                ...
                else prices = `0;${servicePrice};2;${servicePrice};3;${servicePrice}`;
              }
              else {
              ...
              good.push(g.nds_value ? g.nds_value : org.nds_value ? org.nds_value : 15);
              my_array.push(good)
            }
            const CSVString = my_array.join('\n');
            const file = new Date().getTime() + '_ITEMS.csv'
            fs.writeFile('./static/' + file, CSVString, (err) => {
              if (err) {
                instance.send_Error('writing to file withmxik csv', JSON.stringify(err))
              }
              reply.sendFile('./' + file, (err) => {
                instance.send_Error('on sending file withmxik csv', JSON.stringify(err))
              })
              setTimeout(() => {
                fs.unlink('./static/' + file, (err) => {
                  if (err) {
                    instance.send_Error('exported items file unlink withmxik csv', JSON.stringify(err))
                  }
                })
              }, 10000)
            });
  ...
  instance.get('/goods/sales-new/export/:organization/:name', (request, reply) => {
    // instance.authorization(request, reply, (admin) => {
    //_id, sku, name, sold_by, barcode, representation, prices, category, cost
    get_file_new(request, reply, { organization: request.params.organization })
    // })
  })
  instance.get('/goods/sales-mxik/export/:organization/:service', (request, reply) => {
    // instance.authorization(request, reply, (admin) => {
    //_id, sku, name, sold_by, barcode, representation, prices, category, cost
  ...
  instance.get('/item/get-by-barcode/:organization/:barCode', (request, reply) => {
    // instance.authorization(request, reply, (admin) => {
  ...
  const findItemByBarCodeTiin = async (request, reply) => {
    ...
    const organizations = ['5f5641e8dce4e706c062837a', '61ae2917a914c3ba42fc626f']
    instance.goodsSales.aggregate([
      {
        $match: {
      ...
      },
      {
        $project: {
          ...
          price: {
            // $min: [
            // { 
            $min: { "$arrayElemAt": ["$services.prices.price", "$price"] },
            //  },
            // "$price"
            // ]
          },
  ...
  instance.post('/goods/search/mxik',
    { version: '1.0.0', schema: searchMxikSchema },
    async (request, reply) => {
      const body = request.body;
      const goods = await instance.soliqgoods.findOne({
        barcode: { $in: body },
      })
        .lean();
