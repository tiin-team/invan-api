...
module.exports = (instance, _, next) => {
  ...
  const summaryOptions = {
    ...
    schema: {
      ...
      body: {
        type: 'object',
        required: [
          'custom', 'start', 'end', 'services',
          'employees', 'count_type'
        ],
        properties: {
          custom: {
            type: 'boolean',
  ...
  const calculateReportSummary = async (request, reply, admin) => {
    try {
      ...
      const TIME_DIFF = typeof process.env.TIME_DIFF == typeof 5 ? (process.env.TIME_DIFF) : (+process.env.TIME_DIFF);
      ...
      if (!limit) {
        ...
        const totalResult = totalReport && totalReport.length && totalReport[0].count ? totalReport[0].count : 1;
        const skipResult = {
          $skip: limit * (page - 1)
        }
        const limitResult = {
  ...
  instance.post('/reports/summary/:min/:max', summaryOptions, (request, reply) => {
  ...
  instance.post('/reports/summary/:min/:max/:limit/:page', summaryOptions, (request, reply) => {
    instance.oauth_admin(request, reply, (admin) => {
      if (!admin) {
        return reply.error('Access')
      }
