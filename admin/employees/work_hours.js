module.exports = (instance, options, next) => {
  ...
  var get_worked_hours = (request, reply, admin) => {
        ...
        for(var t of timecards) {
          if(emplonameObj[t.employee_id]){
            if((t.clock_out == null || t.clock_out == 0) && emplotimeObj[t.employee_id]) {
              ...
              emplohourObj[t.employee_id].value += (max-t.clock_in)/(1000*60*60)
              total_hours += (max-t.clock_in)/(1000*60*60)
            }
            else {
              if(t.clock_in != null || t.clock_out != null || t.clock_out != 0) {
                emplohourObj[t.employee_id].services.add(t.service_name)
                emplohourObj[t.employee_id].value += (t.clock_out-t.clock_in)/(1000*60*60)
                total_hours += (t.clock_out-t.clock_in)/(1000*60*60)
              }
            }
          }
        }
