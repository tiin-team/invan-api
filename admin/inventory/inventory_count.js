...
const getCountItems = async (request, reply, instance) => {
  try {
    ...
    const goods = await instance.goodsSales.find(query).skip(limit * (page - 1)).limit(limit).sort({ _id: 1 })
    const Answer = []
...
module.exports = (instance, options, next) => {
    ...
    try {
      ...
      for (let i = 0; i < invcount.items.length; i++) {
        if (gObj[invcount.items[i].product_id] != undefined) {
          invcount.items[i].product_id = instance.ObjectId(invcount.items[i].product_id)
          invcount.items[i].product_name = gObj[invcount.items[i].product_id].name
          invcount.items[i].exp_in_stock = gObj[invcount.items[i].product_id].in_stock
  ...
  instance.post('/inventory/count/create', options.version, (request, reply) => {
    instance.oauth_admin(request, reply, (admin) => {
      if (!admin) {
  ...
  instance.post('/inventory/count/update/:id', options.version, (request, reply) => {
    instance.oauth_admin(request, reply, (admin) => {
      if (!admin) {
        return reply.error('Access')
      }
  ...
  async function create_history(request, it, invcount, admin) {
    try {
      ...
      await instance.goodsSales.updateOne({ _id: it.product_id }, updateQuery1, updateQuery2)
      ...
      await instance.create_inventory_history(admin, 'recounted', invcount.p_order, invcount.service, it.product_id, good.cost, it.difference, it.counted, new Date().getTime())
    }
    catch (error) {
  ...
  let complete_count = async (request, reply, admin) => {
    try {
      ...
      let invcount = await instance.inventoryCount.findOne({
        _id: id
      });
      if (!invcount) {
        return reply.fourorfour('invcount')
      ...
      for (let i = 0; i < items.length; i++) {
        if (gObj[items[i].product_id + ''].in_stock != undefined) {
          ...
          items[i].cost_difference = items[i].difference * gObj[items[i].product_id].cost
          await update_count_item(items[i])
        }
      }
      let counts = await instance.inventoryCountItem.find({
        count_id: invcount._id
      });
      var total = {
        total_difference: 0,
  ...
  instance.post('/inventory/count/complete/:id', options.version, (request, reply) => {
    instance.oauth_admin(request, reply, (admin) => {
      if (!admin) {
        return reply.error('Access')
      }
  ...
  instance.get('/inventory/count/complete/:id', options.version, (request, reply) => {
    instance.oauth_admin(request, reply, (admin) => {
      if (!admin) {
        return reply.error('Access')
      }
  ...
  instance.post('/inventory/count/get/:limit/:page', options.version, (request, reply) => {
    instance.oauth_admin(request, reply, (admin) => {
      if (!admin) {
        return reply.error('Access')
      }
      ...
      for (let i = 0; i < countitems.length; i++) {
        ...
        if (gObj[countitems[i].product_id]) {
          ...
          if (gObj[countitems[i].product_id].item_type == 'variant') {
            ...
            if (parent) {
              countitems[i].product_name = `${parent.name} ( ${countitems[i].product_name} )`
            }
          }

          countitems[i].sku = gObj[countitems[i].product_id].sku
          if (countitems[i].counted) {
            countitems[i].difference = countitems[i].counted - countitems[i].exp_in_stock
            count.total_difference += countitems[i].difference
            countitems[i].cost_difference = countitems[i].difference * countitems[i].cost
  ...
  instance.get('/inventory/count/get/:id', options.version, (request, reply) => {
    instance.oauth_admin(request, reply, (admin) => {
      if (!admin) {
        return reply.error('Access')
      }
  ...
  instance.get('/inventory/count/get_items/:id', options.version, (request, reply) => {
    instance.oauth_admin(request, reply, (admin) => {
      if (!admin) {
        return reply.error('Access')
      }
  ...
  async function update_inv_count(history, goodObj, reply) {
    try {
      ...
      if (item) {
        ...
        myvar.cost = (good.cost && typeof good.cost == typeof 5.5) ? good.cost : 0
  ...
  instance.delete('/inventory/count/history/delete/:id', options.version, async (request, reply) => {
    try {
      ...
      if (count) {
        const product = await instance.goodsSales.findById(history.product_id)
