...
module.exports = fp((instance, options, next) => {
	// receive function
	/*
		const receive_purchase = async ({ purch, current_service, purchase_items }, reply, admin) => {
			try {
				if (!purch) return reply.fourorfour('purchase_order')
				const item_ids = [];
				const reqObj = {};
				for (let i = 0; i < purchase_items.length; i++) {
					item_ids.push(purchase_items[i]._id)
					reqObj[purchase_items[i]._id] = purchase_items[i]
					reqObj[purchase_items[i]._id].to_receive = parseFloat(reqObj[purchase_items[i]._id].to_receive)
				}
				const goodsObj = {}
				const pro_ids = []
				const itemObj = {}
				...
				const status = (check_closed) ? "closed" : (is_changed || received > 0) ? 'partially' : 'pending'
				...
				for (const purch_item of purchase_items) {
					...
					await new instance.goodsSaleQueue({
						...
						.save()
						.catch(err => {
							instance.send_Error('Create goodsSaleQueue\n', JSON.stringify(err))
						})
		...
		const createPurchaseOrder = async (request, reply, admin) => {
			...
			if (request.body.items && request.body.items.length > 0) {
				for (let i = 0; i < request.body.items.length; i++) {
					...
					if (
						request.body.items[i].quality != undefined
						&& parseFloat(request.body.items[i].quality)
					) {
						total_count += parseFloat(request.body.items[i].quality)
						if (
							request.body.items[i].purchase_cost != undefined
							&& parseFloat(request.body.items[i].quality)
							&& parseFloat(request.body.items[i].purchase_cost)
						) {
							let amount_quality = parseFloat(request.body.items[i].quality)
								* parseFloat(request.body.items[i].purchase_cost)
					...
					if (
						...
						await instance.goodsSales.updateOne(
							{ _id: request.body.items[i].product_id },
							{ default_purchase_cost: request.body.items[i].purchase_cost }
						)
					}
				...
				purchaseModel.save((err, purch) => {
					...
					instance.purchaseItem.insertMany(purchase_items, (err, purchaseitems) => {
						...
						if (request.body.status == 'returned_order') {
							reply.ok(purch)
							return return_purchase_order(purch, purchase_items, admin)
						}
						if (request.body.status == 'closed') {
							...
							receive_purchase(
								{ purch, current_service: service, purchase_items: purchaseitems },
								reply, admin
							)
						}
	...
	const create_purchase_order = async (request, reply, admin) => {
		...
		const supplier = await instance.adjustmentSupplier
			...
			.lean()
		if (!supplier) return reply.error('Error on finding supplier')

		if (!body.purchase_order_date) {
			body.purchase_order_date = new Date().getTime()
		}
		if (
			body.purchase_order_date != ""
			&& body.expected_on
			&& body.purchase_order_date > body.expected_on
		) {
			return reply.error('Time error')
		}
		...
		const orders = await instance.inventoryPurchase
			.countDocuments({ organization: admin.organization })
			|| 0
		...
		try {
			body.supplier_id = instance.ObjectId(body.supplier_id)
			body.service = instance.ObjectId(body.service)
			body.ordered_by_id = instance.ObjectId(admin._id)
		} catch (error) {
			return reply.error(error.message)
		}
		...
		delete body._id
		if (body.status != 'returned_order' && body.status != 'closed') {
			body.status = 'pending'
		}
		const purchaseModel = new instance.inventoryPurchase(body)
		...
		if (body.items && body.items.length > 0) {
			for (let i = 0; i < body.items.length; i++) {
				delete body.items[i]._id
				body.items[i].purchase_id = instance.ObjectId(purchaseModel._id)
				body.items[i].ordered = body.items[i].quality
				body.items[i].service = instance.ObjectId(body.service)
				if (body.items[i].quality != undefined && parseFloat(body.items[i].quality)) {
					total_count += parseFloat(body.items[i].quality)
					if (
						body.items[i].purchase_cost != undefined
						&& parseFloat(body.items[i].quality)
						&& parseFloat(body.items[i].purchase_cost)
					) {
						let amount_quality = parseFloat(body.items[i].quality) * parseFloat(body.items[i].purchase_cost)
						if (body.items[i].purchase_cost_currency == 'usd') {
							amount_quality = amount_quality * currency.value
						}
						total += amount_quality
					}
				...
				if (parseFloat(body.items[i].purchase_cost) && parseFloat(body.items[i].quality)) {
					body.items[i].purchase_cost = parseFloat(body.items[i].purchase_cost)
					body.items[i].amount = body.items[i].purchase_cost * parseFloat(body.items[i].quality)
					await instance.goodsSales
						.updateOne(
							{ _id: body.items[i].product_id },
							{ default_purchase_cost: body.items[i].purchase_cost },
							{ lean: true },
						)
				}
				purchase_items.push(body.items[i])

			}
			if (!body.additional_cost == undefined) {
				body.additional_cost = []
			}
			for (const r in body.additional_cost) {
				delete body.additional_cost[r]._id
				if (parseFloat(body.additional_cost[r].amount)) {
					let additional_amount = parseFloat(body.additional_cost[r].amount)
					if (body.additional_cost[r].amount_currency == 'usd') {
						additional_amount = additional_amount * currency.value
					}
					total += additional_amount
				}
	...
	instance.post('/inventory/create_purchase_order', { version: '2.0.0' }, (request, reply) => {
		instance.oauth_admin(request, reply, async (admin) => {
			if (admin) {
				return await create_purchase_order(request, reply, admin)
			}
		})
	})
	*/
