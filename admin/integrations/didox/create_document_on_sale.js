...
module.exports = fp(function (fastify, opts, next) {
  ...
  async function a({ client, products }, request, reply) {
    ...
    if (Array.isArray(client.contract_numbers)) {
      clientContract = client.contract_numbers.find(
        (contractNumber) => contractNumber.isDefault,
      );
    }
    ...
    for (const sold_item of receipt.sold_item_list) {
      ...
      didoxProducts[index] = {
        ...
        name: sold_item.product_name,
        catalogcode:
          goodsObj[sold_item.product_id] && goodsObj[sold_item.product_id].mxik
            ? goodsObj[sold_item.product_id].mxik
            : "", // 08471011002000000
