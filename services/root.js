...
function send_to_slack(method, model, er_or_suc) {
  ...
  if (er_or_suc != 'Success')
    axios.post(`https://api.telegram.org/bot${process.env.BOT_TOKEN}/sendMessage?chat_id=${process.env.SMSCHANNEL}&parse_mode=html&text=<strong>${er_or_suc}</strong>  On <b>${method}</b> method   <i>${model}</i>`)
      .then(function (response) { }).catch(function (err) { }).then(function () { })
}

function send_Error(url, error) {
  axios.post(`https://api.telegram.org/bot${process.env.BOT_TOKEN}/sendMessage?chat_id=${process.env.SMSCHANNEL}&parse_mode=html&text=<strong>Error</strong> On <b>${url}</b> url :  \n${error}`)
    .then(function (response) { }).catch(function (err) { }).then(function () { })
}
module.exports = (instance, _, next) => {
  ...
  instance.post('/send_error', { version: '1.0.0' }, (request, reply) => {
    axios.post(`https://api.telegram.org/bot${process.env.BOT_TOKEN}/sendMessage?chat_id=${process.env.SMSCHANNEL}&parse_mode=html&text=${JSON.stringify(request.body)}`).then(() => { }).catch(() => { }).then(() => { })
    reply.ok()
  })
