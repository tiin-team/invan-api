...
module.exports = fp((instance, _, next) => {
  ...
  async function calculateCash(receipt, client, user) {
    try {
      ...
      const phone_number = receipt.cashback_phone.replace('+', '').replace('+', '')

      const organization = await instance.organizations.findById(receipt.organization).lean()
      if (!organization)
        instance.send_Error(`organization not found, _id: ${receipt.organization}. receipt: ${receipt._id}`, `calculateCash`)
      ...
      minus_cash = cashback_pay && cashback_pay.value
        ? cashback_pay.value
        : gift && gift.value
          ? gift.value
          : 0
      ...
      if (!clientsDatabase) {
        ...
        if (!CashBackClient) {
          const text = `\norganization: ${organization._id}` +
            ...
            `\n\nLine: 82`
          //send error to devs
          return instance.send_Error(`${process.env.cash_back_URL}/users?phone=${phone_number}`, text)
        }
        let user_id = new Date().getTime();
        try {
          user_id = CashBackClient.birthDate.replace('.', '').replace('.', '').replace('.', '')
