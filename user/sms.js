...
module.exports = fp((instance, _, next) => {
  instance.decorate(
    "sending_sms_code",
    async (phone_number, sms_code, user = "user", organization = {}) => {
      ...
      if (our_numbers.includes(phone_number) == false) {
        try {
          try {
            ...
            await axios.get(
              `https://api.telegram.org/bot${process.env.BOT_TOKEN}/sendMessage?chat_id=${process.env.SMSCHANNEL}&parse_mode=html&text=${tgText}`,
              {},
            );
          } catch (error) {}
          ...
          await axios
            .post(`https://confirmation.tiinco.uz/send_sms`, {
            ...
            .catch((err) => {
              instance.send_Error(
                "sending error confirmation bot",
                JSON.stringify(err),
              );
            });
          /*
        ...
        try {
          await axios.get(
            `https://api.telegram.org/bot${process.env.BOT_TOKEN}/sendMessage?chat_id=${process.env.SMSCHANNEL}&parse_mode=html&text=${sms_code} is Sms code of ${phone_number} ${user}`,
            {},
          );
        } catch (error) {}
      }
