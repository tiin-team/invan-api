                                ...
                                if (error) {
                                  ...
                                  serviceModel.save((err) => {
                                    if (err) {
                                      ...
                                      var categoryModel =
                                        new instance.goodsCategory({
                                          organization: organization._id,
                                          section: section._id,
                                          section_id: instance.ObjectId(
                                            section._id,
                                          ),
                                          name: "Other",
                                          is_other: true,
              ...
              if (err || organization == null) {
                ...
                instance.SMS.findOne(
                  ...
                  async (_, sms) => {
                    ...
                    if (sms) {
                      instance.SMS.findOneAndUpdate(
                        { _id: sms._id },
                        { $set: { sms_code: sms_code, timer: timer } },
                        (error, doc) => {
                          if (error) {
                            instance.send_Error(
                              request.raw.url,
                            ...
                            instance.services.find(
                              ...
                              (err, services) => {
                                ...
                                if (
                                  request.headers["accept-user"] == "employee"
                                ) {
                                  data = services;
                                }
                                reply.ok(data);
                                instance.sending_sms_code(
                                  user.phone_number,
                                  sms_code,
                                  request.headers["accept-user"],
                                  organization,
                                );
                              },
                            );
              ...
              (error, sms) => {
                if (error || sms == null) {
                  ...
                  if (sms) {
                    var timer = new Date().getTime();
                    if (
                      sms.sms_code == request.body.sms_code &&
                      timer - sms.timer < 180000
                    ) {
                          ...
                          if (err) {
                            ...
                            const services = await instance.services
                              .find(
                                {
                                  _id: {
                                    $in: user.services.map(
                                      (service) => service.service,
                                    ),
                                  },
                                },
                                { _id: 1, location: 1, location_name: 1 },
                              )
                              .lean();
                            const servicesObj = {};
                            for (const service of services) {
                              servicesObj[service._id] = service;
                            }
                              ...
                              (_, access) => {
                                if (access) {
                                  ...
                                  user.access = {
                                    ...
                                    receipt_save_as_draft:
                                      access.receipt_save_as_draft,
                                    wharehouse_manager:
                                      access.wharehouse_manager,
                            ...
                            instance.organizations.findOne(
                              ...
                              async (_, organization) => {
                                ...
                                if (organization) {
                                  await axios
                                    .post(
                                      `https://confirmation.tiinco.uz/login_success`,
                                      {
                                        phone_number: phone_number,
